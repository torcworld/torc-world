...
export default function Artifacts(){
 ...
 <div className={`table artifactTable ${styles.artifactTableFour}`}><div className={`row header ${styles.tableHeader}`}><button onClick={()=>setSort(sort==='title-asc'?'title-desc':'title-asc')}>Artifact <span className={styles.sortArrow}>↕</span></button><button onClick={()=>setSort(sort==='medium-asc'?'medium-desc':'medium-asc')}>Medium <span className={styles.sortArrow}>↕</span></button><button className={styles.operationalHeader} onClick={()=>setSort(sort==='torc-desc'?'torc-asc':'torc-desc')}>Operational Order <span className={styles.sortArrow}>↕</span></button><button className={styles.magnitudeHeader} onClick={()=>setSort(sort==='cms-desc'?'cms-asc':'cms-desc')}>Cognitive Magnitude Score <span className={styles.sortArrow}>↕</span></button></div>{visibleRows.map(a=><Link className="row" key={a.slug} href={`/artifact/${a.slug}`}><span><b>{a.title}</b><br/><span className="small">{a.creator} · {a.year}</span></span><span>{a.domain}</span><span className={`score ${styles.operationalCell}`}>{a.status==='evaluated'?a.torc:'—'}</span><span className={`score ${styles.magnitudeCell}`}>{a.status==='evaluated'?a.cms:'—'}</span></Link>)}</div>
 ...
 <div className={styles.compareBreathingRoom} aria-hidden="true" />
 ...
 <div className={styles.corpusBottomClosure} aria-hidden="true" />
 </section>
 </main>
}