...
export default function ArtifactAnalysis(p:Props){const dimensions=p.dimensions??[];return <main className={styles.page}>
<header className={styles.hero}><div className="eyebrow">{p.domain} · {p.year} · TORC analysis</div><h1>{p.title}</h1><p className={styles.creator}>{p.creator}</p><div className={styles.verdictGrid}><div><span>Operational Order</span><strong>{p.order}</strong><small>{p.orderName}</small></div><div><span>Cognitive Magnitude</span><strong>{typeof p.cms==='number'?p.cms:'—'}</strong><small>{typeof p.cms==='number'?(p.range??'Production CMS'):'CMS pending further evaluation'}</small></div></div></header>
...
<section className={styles.cmsSection}><div className={styles.kicker}>Cognitive architecture</div><h2 className={styles.dimensionsTitle}>The six-dimensional model</h2>{dimensions.length?<><p className={styles.cmsIntro}>See how TORC’s Cognitive Magnitude Score (CMS) is carried across the artifact.</p><div className={styles.dimensions}>{dimensions.map(d=><details className={styles.dimension} key={d.key}><summary><span className={styles.dimKey}>{d.key}</span><span className={styles.dimName}><b>{d.name}</b><small>Definition + artifact analysis</small></span><span className={styles.bar} aria-hidden="true"><span style={{width:`${Number(d.score)*10}%`}} /></span><strong>{d.score}</strong></summary><div className={styles.dimBody}><p><b>In plain language.</b> {defs[d.key][1]}</p><p><b>Why this artifact scores {d.score}.</b> {d.artifact}</p></div></details>)}</div></>:<p className={styles.cmsIntro}>Operational Order is settled independently of magnitude. The public CMS and six-dimensional profile will be added after the magnitude estimate has been stabilized across further evaluations.</p>}</section>
